...
async function main() {
  try {
    ...
    const octoberResult = await pool.query(
      ...
      WHERE f."orgId" = $1
        AND f.date >= '2025-10-01'::date
        AND f.date < '2025-11-01'::date
      ORDER BY f.date, f.type
    `,
      [orgId]
    )

    const octoberData = octoberResult.rows
    ...
    const novemberResult = await pool.query(
      `
      SELECT 
        date, type, amount, description, 
        c.name as client_name
      ...
      WHERE f."orgId" = $1
        AND f.date >= '2025-11-01'::date
        AND f.date < '2025-12-01'::date
      ORDER BY f.date, f.type
    `,
      [orgId]
    )

    const novemberData = novemberResult.rows
