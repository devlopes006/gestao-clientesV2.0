      ...
      {items.map((item) => {
        ...
        return (
          <Link
            ...
            href={item.href}
            className={
              `group relative gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm rounded-lg inline-flex items-center transition-all duration-300 flex-shrink-0 whitespace-nowrap font-medium overflow-hidden` +
              (item.destructive
          ...
          >
            ...
            {/* Icon container */}
            <div className={`relative z-10 p-0.5 rounded-md transition-all duration-300 ${active ? 'bg-blue-500/20' : 'group-hover:bg-slate-700/50'
              }`}>
              {Icon && <Icon className={`h-4 w-4 transition-transform duration-300 ${active ? 'scale-110' : 'group-hover:scale-110'
                }`} />}
            </div>
