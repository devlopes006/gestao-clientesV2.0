...
export function TransacoesTab() {
  ...
  const getStatusBadge = (status: TransactionStatus) => {
    const variants: Record<TransactionStatus, { styles: string; label: string }> = {
      CONFIRMED: {
        styles: 'bg-gradient-to-r from-violet-500/20 to-purple-500/20 text-violet-300 border border-violet-500/30 shadow-sm shadow-violet-500/20',
        label: 'Confirmado'
      },
      PENDING: {
    ...
    <section className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-2">
          <p className="text-xs font-bold text-violet-400 uppercase tracking-widest">Movimentações</p>
          <h2 className="text-3xl font-black bg-gradient-to-r from-violet-400 via-purple-400 to-indigo-400 bg-clip-text text-transparent drop-shadow-lg">Transações</h2>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            ...
            className="bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 shadow-lg shadow-violet-500/30"
          >
            Nova transação
          </Button>
        </div>
      ...
      <div className="rounded-2xl border border-violet-500/20 bg-gradient-to-br from-slate-900 via-violet-950/10 to-slate-900/90 backdrop-blur-xl p-6 shadow-2xl">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-5">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 rounded-xl border border-slate-700 bg-slate-900/50 px-3 py-2.5 focus-within:border-violet-500/50 focus-within:ring-2 focus-within:ring-violet-500/20 transition-all">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="h-11 w-full rounded-xl border border-slate-700 bg-slate-900/50 px-4 text-sm text-slate-100 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition-all"
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Input
                ...
                className="h-11 rounded-xl border-slate-700 bg-slate-900/50 text-slate-100 focus:border-violet-500 focus:ring-violet-500/20"
              />
            </div>
            <div className="space-y-2">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-700/30 pt-4">
          <span className="text-sm font-semibold text-slate-300">
            Mostrando <span className="text-violet-400">{filteredTransactions.length}</span> de <span className="text-violet-400">{totalCount}</span> transações
          </span>
      ...
      <div className="space-y-4">
        ...
        {loading && (
          <div className="flex items-center gap-3 rounded-2xl border border-violet-500/30 bg-gradient-to-br from-violet-500/10 to-slate-900/90 backdrop-blur-xl px-6 py-8 shadow-xl">
          ...
          <>
            <div className="space-y-3 md:hidden">
              {filteredTransactions.map((transaction) => (
                <div key={transaction.id} className="group rounded-2xl border border-slate-700/50 bg-gradient-to-br from-slate-900/80 to-slate-800/80 backdrop-blur-xl p-5 shadow-lg hover:shadow-xl hover:border-violet-500/30 transition-all duration-300">
            ...
            <div className="hidden md:block overflow-hidden rounded-2xl border border-violet-500/20 bg-gradient-to-b from-slate-900 to-slate-900/80 shadow-2xl backdrop-blur-xl">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-left text-sm">
                  ...
                  <tbody>
                    {filteredTransactions.map((transaction) => (
                      <tr key={transaction.id} className="group border-b border-slate-700/30 last:border-b-0 hover:bg-gradient-to-r hover:from-violet-500/5 hover:to-purple-500/5 transition-all duration-300 hover:shadow-lg">
                        <td className="px-5 py-4 text-sm text-slate-300 group-hover:text-slate-200 transition-colors">
                          {new Date(transaction.date).toLocaleDateString('pt-BR')}
                        </td>
                        <td className="px-5 py-4">
                          <div className="flex flex-col gap-1">
                            <span className="font-semibold text-slate-100 group-hover:text-violet-400 transition-colors line-clamp-1">{transaction.description}</span>
                            {transaction.category && (
                        ...
                        <td className="px-5 py-4">
                          <div className="flex flex-wrap items-center gap-2">
                            {getTypeBadge(transaction.type)}
                            <Badge variant="outline" className="text-xs border-slate-600 text-slate-300 group-hover:border-violet-500/30 transition-colors">{transaction.subtype.replace('_', ' ')}</Badge>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <Button
                            ...
                            size="sm"
                            onClick={() => handleViewTransaction(transaction.id)}
                            className="text-violet-400 hover:text-violet-300 hover:bg-violet-500/10 transition-all duration-300"
                          >
                            Detalhes
                          </Button>
                        </td>
